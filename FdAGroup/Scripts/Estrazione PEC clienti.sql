...
--[Denominazione sociale], con sede legale in [Città] - [CAP], Via […] n. […], Codice Fiscale e Partita IVA n. […], in
--persona del rappresentante legale munito dei necessari poteri per la sottoscrizione del presente atto

CREATE TABLE QTEMP.CLIDET (CDCL CHAR(7)) 
-- TROVO TUTTI I CLI I CUI PVE SONO CLASSIFICATI COM DET
INSERT INTO QTEMP.CLIDET 
SELECT 
--COUNT(*) 
DISTINCT E§CCON
FROM SMEDATFDA.BRENTI0F WHERE E§TRAG='PVE' AND E§STAT='10' AND E§COD8='DET'                  
...
WHERE S§TRAG='CLI' AND S§TEST='£51'  AND S§CRAG IN (SELECT CDCL FROM QTEMP.CLIDET)
AND S§LIBE LIKE '%@%' 
--AND S§LIBE LIKE '%@%'
) 
SELECT E§STAT  "Stato Ente", 
E§TRAG "Tipo",
E§NMNE "Sigla",
E§CRAG "Codice" , 
E§RAGS "Denominazione sociale",
E§RAGA "Denominazione sociale aggiuntiva", 
E§LOCA "Città",
E§INDI "Indirizzo", 
E§CAPA "CAP", 
E§PROV "Provincia",  
E§CPAI "Partita IVA", 
E§COFI "Codice Fiscale", 
X.PEC  "PEC"
FROM SMEDATFDA.BRENTI0F INNER JOIN X ON E§CRAG=X.S§CRAG 
WHERE E§TRAG='CLI' --AND E§COD8='DET'  
      AND E§DINV <= YEAR(CURRENT_DATE)*10000
                  + MONTH(CURRENT_DATE)*100
                  + DAY(CURRENT_DATE)
      AND E§DFNV >= YEAR(CURRENT_DATE)*10000
...
SELECT DISTINCT E§CCON FROM SMEDATFDA.BRENTI0F WHERE                   
