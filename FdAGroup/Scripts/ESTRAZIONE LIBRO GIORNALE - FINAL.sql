...
-- VERSIONE DEL 12/04/2024 A CUI E' STATA AGGIUNTA LA COLONNA  GL_USER_DESCRIPTION
SELECT
...
G.TTDESC || '|' ||  -- GL_DESCRIPTION,
T5DDOR || '|' ||    -- GL_DOCUMENT_DATE,
T5USIN || '|' ||    -- GL_USER_REGISTRATION,
X.TTDESC            -- GL_USER_DESCRIPTION 
FROM  
--SMEDATFDA.C5TREG0F INNER JOIN SMEDATFDA.C5RREG0F ON R5PROG=T5PROG INNER JOIN SMEDATGRU5.TABELG0F G  ON G.TTSETT='C5B' AND G.TTELEM=R5CONT INNER JOIN SMEDATFDA.TABEL00F X -- FDA
--SMEDATLIZ.C5TREG0F INNER JOIN SMEDATLIZ.C5RREG0F ON R5PROG=T5PROG INNER JOIN SMEDATGRU5.TABELG0F G  ON G.TTSETT='C5B' AND G.TTELEM=R5CONT INNER JOIN SMEDATLIZ.TABEL00F X  -- LIZZI  
SMEDATBND.C5TREG0F INNER JOIN SMEDATBND.C5RREG0F ON R5PROG=T5PROG INNER JOIN SMEDATGRU5.TABELG0F G  ON G.TTSETT='C5B' AND G.TTELEM=R5CONT INNER JOIN SMEDATBND.TABEL00F X -- SIPA                
ON X.TTSETT='B£U' AND X.TTELEM=T5USIN 
  WHERE T5AZIE IN 
--('01', '02', '06', '07', '11') -- FDA
--('09')  -- LIZZI                   
('10', '12')  -- SIPA
